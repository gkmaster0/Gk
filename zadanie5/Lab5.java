	...
	private void draw(GL2 gl2) {
		...
		if(objectNumber==1) //Kork
		{
			...
			for (i = 0.0f; i<= (java.lang.Math.PI * 2f * n); i += distance)
			{
				...
				gl2.glColor3f(0.0f, 0.5f, 0.0f);
				gl2.glVertex3f(x, y, z);
				y += 0.08;
			}
			gl2.glEnd();
		}
		else if(objectNumber==2) //rysowanie
		{
			gl2.glLineWidth(3);
			...
			for (i = 0; i< n*2; i += 1) //Piramida
			{
				x = (float) Math.sin(i * 2 * Math.PI / n)*rozm;
				z = (float) Math.cos(i * 2 * Math.PI / n)*rozm;
				gl2.glColor3f(0.0f, 0.5f, 0.0f);
				gl2.glVertex3f(x, y, z);
				if(i%2==0) {
